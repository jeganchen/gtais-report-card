/**
 * 学年同步API
 * 从PowerSchool获取学年/学期数据并保存到数据库
 */

import { NextRequest, NextResponse } from 'next/server';
import { settingsRepository, termRepository, schoolRepository, syncLogRepository } from '@/lib/database/repositories';

// PowerSchool返回的学期数据结构
interface PSTermsRecord {
  id: number;
  name: string;
  tables: {
    terms: {
...
interface PSSchoolYearsResponse {
  name: string;
  record: PSTermsRecord[];
  '@extensions'?: string;
}

/**
 * POST /api/sync/school-years - 同步学年/学期数据
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const apiUrl = `${baseUrl}/ws/schema/query/org.infocare.school.years`;
    ...
    const data: PSSchoolYearsResponse = await response.json();
    console.log('PowerSchool response:', JSON.stringify(data, null, 2));
    ...
    // 设置当前学年（基于当前日期，优先选择isYearRec=true的记录）
    const now = new Date();
    ...
    await syncLogRepository.complete(log.id, termsToSave.length, {
      schoolId: psSchoolId,
      terms: termsToSave.map(t => t.name),
    });
...
/**
 * GET /api/sync/school-years - 获取已同步的学年/学期列表
 */
export async function GET() {
  try {
    // 优先返回学年记录
    const years = await termRepository.findAllYears();
    const currentTerm = await termRepository.findCurrent();
    
    // 如果没有学年记录，返回所有学期
    const terms = years.length > 0 ? years : await termRepository.findAll();
    
    return NextResponse.json({
      years: terms.map((t: typeof terms[number]) => ({
        id: t.id,
        psId: t.psId,
        name: t.name,
        abbreviation: t.abbreviation,
    ...
    return NextResponse.json(
      { error: 'Failed to get terms', years: [] },
      { status: 500 }
    );
  }
}
